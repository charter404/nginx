...
I want to deploy my website with reverseproxy with path are
...
share the cnfigurstin for it

You can configure Nginx to act as a reverse proxy with path-based routing.
Each path (/home, /new, /contact) will forward requests to different backend services.
...
Nginx Reverse Proxy
   │
   ├── /home     → backend1 (port 9001)
   ├── /new      → backend2 (port 9002)
   └── /contact  → backend3 (port 9003)

1️⃣ Backend servers (example)

Assume you run three services:

localhost:9001 → Home service
localhost:9002 → New page service
localhost:9003 → Contact service
...
Create or edit your site config:
...
http://mysite.local/home
http://mysite.local/new
http://mysite.local/contact

Requests will go to:

/home    → localhost:9001
/new     → localhost:9002
/contact → localhost:9003

6️⃣ Request flow diagram
...
⚠️ Important detail (trailing slash)
...
The trailing / ensures correct path rewriting.
7️⃣ Optional: redirect root /

If someone visits /, redirect to /home:
...
mysite.local/home     → Home service
mysite.local/new      → New page service
mysite.local/contact  → Contact service

If you want, I can also show you the production-grade Nginx config used in microservice architectures where 10–20 paths route to different backend services, including load balancing + SSL + caching.
